...
CHOOSING, CHOOSE_MENTAL, TYPING_CHOICE = range(3)
...
def custom_choice_final(update, context):
    if fisica == 1 and sabiduria == 1 and mental == 1:
        update.message.reply_text(
            'Algun día pondremos esta prueba, Tendras que acudir a Jorge y Marta',reply_markup=markup)
        return CHOOSING
...
def done(update, context):
    ...
    if fisica == 1 and mental == 1 and sabiduria == 1 and final == 1:
        update.message.reply_text(
            'Enhorabuena, has superado las pruebas, para celebrarlo vamos a tomar algo hasta el gran Capitán, te lo has ganado CRACK!!!',)
...
def main():
    ...
    updater = Updater(
        "1398887507:AAGwKNPRKerCuq3n01DUR3x1lWxIQQxkqnA", use_context=True)
